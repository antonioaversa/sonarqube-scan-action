...
if [[ "$RUNNER_OS" == "Linux" && "$RUNNER_ARCH" == "X64" ]]; then
     ...
     echo "::error $RUNNER_OS $RUNNER_ARCH not supported"
     exit 1
fi
...
if command -v wget &> /dev/null; then
    ...
    DOWNLOAD_ARGS_BEFORE_URL="--user-agent=sonarqube-scan-action"
elif command -v curl &> /dev/null; then
    DOWNLOAD_COMMAND="curl"
    DOWNLOAD_ARGS_BEFORE_URL="--user-agent sonarqube-scan-action --output"
    ...
    echo "::error Neither wget nor curl found on the machine"
    exit 1
fi
...
$DOWNLOAD_COMMAND @DOWNLOAD_ARGS $SCANNER_FILE_NAME $SCANNER_URI
